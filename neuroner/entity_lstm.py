...
class EntityLSTM(object):
    ...
    def load_pretrained_token_embeddings(self, sess, dataset, parameters, 
        ...
        """
            load token embeddings from lmdb file
        """
        ...
        if token_to_vector == None:
            ...
            token_to_vector = LruCachedLmdbEmbeddingsReader(parameters['token_pretrained_embedding_filepath'])
        ...
        def get_token_vector(token):
            try:
                return token_to_vector.get_word_vector(token)
            except MissingWordError:
                return

        for token in dataset.token_to_index.keys():
            if get_token_vector(token) is not None:
                initial_weights[dataset.token_to_index[token]] = get_token_vector(token)
                number_of_token_original_case_found += 1
            elif parameters['check_for_lowercase'] and get_token_vector(token.lower()) is not None:
                initial_weights[dataset.token_to_index[token]] = get_token_vector(token.lower())
                number_of_token_lowercase_found += 1
            elif parameters['check_for_digits_replaced_with_zeros'] and get_token_vector(re.sub(r'\d', 
                '0', token)) is not None:
                initial_weights[dataset.token_to_index[token]] = get_token_vector(re.sub(r'\d', 
                '0', token))
            ...
            and get_token_vector(re.sub('\d', '0', token.lower())) is not None:
                initial_weights[dataset.token_to_index[token]] = get_token_vector(re.sub('\d', 
                '0', token.lower()))
