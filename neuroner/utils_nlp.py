...
def get_token_vector(token_to_vector,token):
    """ get vector from LmdbEmbedding """
    try:
        return token_to_vector.get_word_vector(token)
    except MissingWordError:
        return

def is_token_in_pretrained_embeddings(token, token_to_vector, parameters):
    _get_token_vector = functools.partial(get_token_vector,token_to_vector)
    return _get_token_vector(token) is not None or \
        parameters['check_for_lowercase'] and _get_token_vector(token.lower()) is not None or \
        parameters['check_for_digits_replaced_with_zeros'] and _get_token_vector(re.sub('\d', '0', token)) is not  None or \
        parameters['check_for_lowercase'] and parameters['check_for_digits_replaced_with_zeros'] and _get_token_vector(re.sub('\d', '0', token.lower())) is not None 
